...
composeANF :: (ValidDesc a, ValidDesc b, ValidDesc c) => ANF b c -> ANF a b -> ANF a c
...
comp :: (ValidDesc a, ValidDesc b, ValidDesc c) => NoLoop a b -> NoLoop b c -> NoLoop a c
...
compTwoCompose :: ValidDesc c => CompTwo a c -> NoLoop a c
...
compTwoPar :: (ValidDesc a, ValidDesc a') => CompTwo a b -> CompTwo a' b' -> CompTwo (P a a') (P b b')
compTwoPar (C2 a b) (C2 a' b') = C2 (parSimplify a a') (parSimplify b b')

compSimplify :: (ValidDesc a, ValidDesc b, ValidDesc c) => NoComp a b -> NoComp b c -> NoLoop a c
-- SIMPLIFICATION: Id >>> f ==> f <== f >>> Id
...
moveIdInwards :: (ValidDesc a, ValidDesc b, ValidDesc c) => NoComp a b -> NoComp b c -> CompTwo a c
...
par :: (ValidDesc a, ValidDesc b, ValidDesc a', ValidDesc b') => NoLoop a b -> NoLoop a' b' -> NoLoop (P a a') (P b b')
...
parSimplify :: (ValidDesc a, ValidDesc a', ValidDesc b, ValidDesc b') => NoComp a b -> NoComp a' b' -> NoComp (P a a') (P b b')
...
arr :: ValidDesc b => (Val a -> Val b) -> ANF a b
...
first :: (ValidDesc a, ValidDesc b, ValidDesc c) => ANF a b -> ANF (P a c) (P b c)
first = (*** id)
second :: (ValidDesc a, ValidDesc b, ValidDesc c) => ANF a b -> ANF (P c a) (P c b)
second = (id ***)

loop :: (ValidDesc a, ValidDesc b, ValidDesc c) => ANF (P a c) (P b c) -> ANF a b
